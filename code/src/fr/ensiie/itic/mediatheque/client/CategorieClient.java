...
public final class CategorieClient implements Serializable {
	...
	public CategorieClient(final String nom, final int max, final double cot, final double coefDur,
			final double coefTar, final boolean codeReducAct) {
		...
		if (coefDur <= 0) {
			throw new IllegalArgumentException("le coefficient de duree doit etre positif ou 0");
		}
		if (coefTar < 0) {
			throw new IllegalArgumentException("le coefficient de tarif doit etre positif ou 0");
		}
		nomCat = nom;
		nbEmpruntMax = max;
		cotisation = cot;
	...
	public boolean invariant() {
		return nomCat != null && !nomCat.equals("") && nbEmpruntMax >= 0 && cotisation >= 0 && coefDuree > 0
				&& coefTarif >= 0;
	}
	...
	/**
	 * Modification du booleen rendant le code de reduction actif dans les clients
	 * de cette catégorie.
	 * 
	...
	/**
	 * Retourne true si le code de reduction est actif dans les clients de cette
	 * catégorie.
	 * 
	 * @return true si vrai.
	 */
	...
	public boolean equals(final Object obj) {
		...
		if (!(obj instanceof CategorieClient)) {
			return false;
		}
